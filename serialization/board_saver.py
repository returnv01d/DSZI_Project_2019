class BoardSaver:
    ...
    def save_board_to_file(board, filepath):
        ...
        file.write("\n")
        file.write(str(len(board.kitchen.orders)))
        file.write("\n")


        for j in range (0, len(board.kitchen.orders)):
            file.write("{0} {1}".format(str(board.kitchen.orders[j].table_id), str(board.kitchen._orders[j].order_name)))
            file.write("\n")
